...
class Ads_Widget extends WP_Widget {
    ...
    function Ads_Widget() {
        ...
        parent::__construct($control_ops['id_base'], 'PPO: Ads sidebar', $widget_ops, $control_ops);
    }

    function form($instance) {
        $defaults = array('title' => 'Ads', 'image_url' => '', 'target_url' => '');
        ...
        $display = '<p><label for="' . $this->get_field_id('title') . '">Title:</label>
			...
			<textarea id="' . $this->get_field_id('custom_code') . '" name="' . $this->get_field_name('custom_code') . '" class="widefat" rows="5">' . $instance['custom_code'] . '</textarea>
		</p>';
        print $display;
    }
    ...
    function widget($args, $instance) {
        ...
        if (!empty($custom_code)) {
            ...
            $display = '<a href="' . $target_url . '" rel="nofollow"><img src="' . $image_url . '" /></a><!--END ADS-->';
        }
        print $display;
        print $after_widget;
    }
