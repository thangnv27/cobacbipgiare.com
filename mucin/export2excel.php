...
ini_set('error_log', $_SERVER['DOCUMENT_ROOT'] . '/mrluu/php-errors.log'); // path to server-writable log file
...
$objPHPExcel->getProperties()->setCreator("MR.LUU")
                            ->setLastModifiedBy("MR.LUU")
                            ->setTitle("MR.LUU ORDERS")
                            ->setSubject("MR.LUU ORDERS")
                            ->setDescription("Xuất danh sách các đơn hàng đã được phê duyệt")
                            ->setKeywords("")
                            ->setCategory("");
...
header('Content-Disposition: attachment;filename="PPO - Don hang.xlsx"');
